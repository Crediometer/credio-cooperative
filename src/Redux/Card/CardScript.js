...
export const init = () => {
  return async (difpch) => {
    if (dispatch == undefined) return (dispatch = difpch);
  };
};
// let button = document.getElementById('MBtn');
// let buttonScan = document.getElementById('request-device');
...
var mService = new main.QPOSService();
...
QPOSServiceListenerImpl.prototype.onReturnDoInputCustomStr = function (
  msg,
  name
) {
...
QPOSServiceListenerImpl.prototype.onDoTradeResult = function (msg, msg1) {
  ...
  if (msg == "ICC") {
    ...
    mService.getNFCBatchData(
      function onSuccess(nfcBatchData) {
        // trasactionData.innerText =
        // "NFCBatchData: " + "\r" + nfcBatchData.toUpperCase();
      },
...
QPOSServiceListenerImpl.prototype.onReturnGetEMVListResult = function (
  aidString
) {
...
QPOSServiceListenerImpl.prototype.onReturnCustomConfigResult = function (
  flag,
  msg
) {
...
QPOSServiceListenerImpl.prototype.onRequestUpdateWorkKeyResult = function (
  flag
) {
...
QPOSServiceListenerImpl.prototype.onReturnApduResult = function (
  result,
  apdu,
  apduLen
) {
  console.log(
    "onReturnApduResult(boolean result,String apdu,int adpuLen)----------" +
      result +
      "\n" +
      apdu +
      "\n" +
...
QPOSServiceListenerImpl.prototype.onReturnPowerOffNFCResult = function (
  result
) {
...
QPOSServiceListenerImpl.prototype.onReturnPowerOnNFCResult = function (
  result,
  ksn,
  atr,
  atrLen
  ...
  console.log(
    "onReturnPowerOnNFCResult(boolean result,String ksn,String atr,int atrLen)-------" +
      result +
      "\n" +
      ksn +
      "\n" +
...
// export const buttonScan = () => {
//   return async (dispatch) => {
//     main.connectToDeviceUSB();
//     mService.setCommunicationMode(main.CommunicationMode.USB);
...
export function startTrade(amount) {
  console.log("call me uncle");
  return async (dispatch) => {
...
function transactionTypeConvert(str) {
  console.log("Str:" + str);
  if (str == "0") {
    return main.TransactionType.GOODS;
  } else if (str == "1") {
    return main.TransactionType.SERVICES;
  } else if (str == "2") {
    return main.TransactionType.CASH;
  } else if (str == "3") {
    return main.TransactionType.CASHBACK;
  } else if (str == "4") {
    return main.TransactionType.INQUIRY;
  } else if (str == "5") {
    return main.TransactionType.TRANSFER;
  } else if (str == "6") {
    return main.TransactionType.ADMIN;
  } else if (str == "7") {
    return main.TransactionType.CASHDEPOSIT;
  } else if (str == "8") {
    return main.TransactionType.PAYMENT;
  } else if (str == "9") {
    return main.TransactionType.PBOCLOG;
  } else if (str == "10") {
    return main.TransactionType.SALE;
  } else if (str == "11") {
    return main.TransactionType.PREAUTH;
  } else if (str == "12") {
    return main.TransactionType.ECQ_DESIGNATED_LOAD;
  } else if (str == "13") {
    return main.TransactionType.ECQ_UNDESIGNATED_LOAD;
  } else if (str == "14") {
    return main.TransactionType.ECQ_CASH_LOAD;
  } else if (str == "15") {
    return main.TransactionType.ECQ_CASH_LOAD_VOID;
  } else if (str == "16") {
    return main.TransactionType.UPDATE_PIN;
  } else if (str == "17") {
    return main.TransactionType.REFUND;
  } else if (str == "18") {
    return main.TransactionType.SALES_NEW;
  } else if (str == "19") {
    return main.TransactionType.LEGACY_MONEY_ADD;
  } else if (str == "20") {
    return main.TransactionType.NON_LEGACY_MONEY_ADD;
  } else if (str == "21") {
    return main.TransactionType.BALANCE_UPDATE;
  }
}
...
function readLine() {
  ...
  ipt.onchange = () => {
    ...
    reader.onload = () => {
      var content = main.byteArray2Hex(reader.result);
...
//发现蓝牙设备
function DiscoverDevice(dispatch) {
  //过滤出我们需要的蓝牙设备
  //过滤器
  var options = {
    filters: [
...
//连接到蓝牙设备
function ConnectDevice(dispatch) {
...
//发现蓝牙设备的服务和特性
function DiscoverService(dispatch) {
      ...
      service.getCharacteristics().then((Characteristics) => {
        ...
        Characteristics.forEach((Characteristic) => {
          ...
          if (Characteristic.uuid == MPOS_VALUE) {
            main.setWriter(Characteristic);
            MPOS_DATA_Ready = true;
            mService.setCommunicationMode(main.CommunicationMode.BLUETOOTH);
            new main.webBluetooth(main.mOnResult, main.writeChar);
          } else if (Characteristic.uuid == notify_uuid) {
            MPOS_Notify_State = Characteristic;
            main.registNotify(MPOS_Notify_State);
          }
          if (
            ServicesDiscovered == ServiceSum &&
            CharacteristicsDiscovered == CharacteristicsSum
          ) {
            console.log("===>服务搜索完成<===");
            //更新UI的信息
            if (MPOS_DATA_Ready) {
...
function UpdateUI(dispatch) {
  //是否已连接
  if (Connected) {
    // document.getElementById("UI_Connected").innerHTML =
    //   "Connect Status：Connected";
    // document.getElementById("MBtn").innerHTML = "Disconnect";
    // document.getElementById("UI_DeviceType").innerHTML =
    //   "Device Type：" + Connected_Device.name;
  } else {
    // document.getElementById("UI_Connected").innerHTML =
